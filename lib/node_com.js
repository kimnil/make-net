...
exports.createAndConfigPeer = function(token, aEndLabel, zEndLabel, aEndIp, zEndIp) {
    const promises = [];
    ...
    const createPromise = internalCreatePeer(token, aEndLabel, zEndLabel, aEndIp, zEndIp);
    ...
    createPromise.then(() => {
        ...
        // Z-End
        Q.delay(1000).then(() => {
            return setConfig(token, zEndIp, "mib/topo/peer/peer:" + zEndLabel, "topoPeerLocalLabel=" + zEndLabel);
        }).then(() => {
            return Q.delay(1000).then(() => {
                promises.push(setConfig(token, zEndIp, "mib/topo/peer/peer:" + zEndLabel, "topoPeerRemoteLabel=" + aEndLabel));
            });
        }).then(() => {
            return Q.delay(1000).then(() => {
                promises.push(setConfig(token, zEndIp, "mib/topo/peer/peer:" + zEndLabel, "topoPeerRemoteIpAddress=" + aEndIp));
            });
        }).then(() => {
            return Q.delay(1000).then(() => {
                promises.push(setConfig(token, zEndIp, "mib/topo/peer/peer:" + zEndLabel, "topoPeerRemoteSubrack=" + aSubrack));
            });
        }).then(() => {
            return Q.delay(1000).then(() => {
                promises.push(setConfig(token, zEndIp, "mib/topo/peer/peer:" + zEndLabel, "topoPeerRemoteSlot=" + aSlot));
            });
        }).then(() => {
            return Q.delay(1000).then(() => {
                promises.push(setConfig(token, zEndIp, "mib/topo/peer/peer:" + zEndLabel, "topoPeerRemotePort=" + aPort));
            });
        });
    });
