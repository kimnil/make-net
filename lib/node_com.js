...
const getSessionTokenInternal = function(ip, retriesLeft) {
   ...
   if (retriesLeft == 0) {
    ...
    doHttp(options)
        ...
        .catch((err) => {
            Q.delay(1000).then(() => {
                getSessionTokenInternal(ip, --retriesLeft)
                .done((res) => {
...
function internalCreateEquipment(ip, token, boards) {
    ...
    boards.forEach((board) => {
        ...
        const eqPromise = doHttp(options);
        const confirmPromise = eqPromise.then(() => {
            return Q.delay(1000).then(confirmCreated(ip, token, board));
        });
    ...
    httpPromise
        .then((body) => {
            if (body.indexOf("equipmentBoardIndex") === -1) {
                return internalCreateEquipment(ip, token, [board]);
            } else {
