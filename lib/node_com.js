...
function internalCreatePeer(token, aEnd, zEnd, aEndIp, zEndIp) {
    ...
    console.log("internalCreatePeer");
...
exports.configureBoard = function(token, ip, boards) {
    console.log("Configure board equipment", ip, boards);
    const promises = [];

    for(var i = 0, sleepFactor = 1; i < boards.length; i++) {
        for(var j = 0; j < boards[i].settings.length; j++) {
            promises.push(internalConfigureBoard(ip, token, boards[i].settings[j], 1000 * sleepFactor));
        }
    }
...
function internalConfigureBoard(ip, token, settingsPath, sleepInMs) {
    console.log("internalConfigureBoard", settingsPath);
    const options = {
        method: 'GET',
        hostname: ip,
...
function confirmCreated(ip, token, board) {
    var options = {
        method: 'GET',
        hostname: ip,
        path: '/mib/eq/board/' + board.toLowerCase() + '/get.json',
        headers: {
            cookie: "sessionId=" + token
        }
    };

    const httpPromise = doHttp(options);

    httpPromise
        .then((body) => {
            if (body.indexOf("equipmentBoardIndex") === -1) {
                return Q.delay(2000).then(() => {
                    internalCreateEquipment(ip, token, [board]);
                });
            } else {
...
function setConfig(token, ip, partialPath, arg) {
    console.log("setConfig", ip, partialPath, arg);
